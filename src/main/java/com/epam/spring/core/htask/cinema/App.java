...
public class App {
    ...
    public void generateTestDatabaseTickets(int countTickets)
    {
        ...
        for(int i=0; i<=countTickets; i++)
        {
            ...
            User user = users.get(rand.nextInt(3)+1);
     ...
     public static void main(String[] args) throws ParseException
     {
         ...
         {
             System.out.println(++num + " >> "+t.getEvent()+" - > "+t.getUser().getName()+" - "+t.getSeat()+" = "+t.getPrice());
         }
          
         //Выводим историю по User-у 
         num = 0;
         List<Ticket> userTickets = app.users.getBookedTickets(app.users.get(1), app.bookingService.getTickets());
